...
/**
 * Seeder de dades de demostracio per a la botiga de bambes.
 *
 * Usat exclusivament en entorn de desenvolupament i demostracio.
 * No esta pensat per a dades de produccio.
 *
 * Crea tota l'estructura necessaria per a Lunar dins una transaccio atomica:
 * - Idioma per defecte (segons app.locale).
 * - Moneda EUR com a moneda per defecte.
 * - Grup de clients 'Default' (requerit per als preus).
 * - Tipus de producte 'Bambes'.
 * - Classe fiscal 'Standard' (requerida per ProductVariant).
 * - 4 col·leccions: Trail, Asfalt, Pista, Mixt.
 * - Opcions de producte: Talla (40-44) i Color (Negre, Blanc, Blau).
 * - 10 productes demo (Nike, ASICS, HOKA, Adidas, Salomon, Saucony,
 *   La Sportiva, Brooks, New Balance, Mizuno) amb 6 variants cadascun
 *   (talla 41/42/43 x color Negre/Blanc), preus en centims EUR i
 *   associacio a col·leccions.
 *
 * S'adapta dinamicament a l'esquema de la BD detectant columnes
 * opcionals (handle, default) a les taules de Lunar, permetent
 * compatibilitat amb diferents versions de migracions.
 *
 * Us: php artisan db:seed --class=LunarBambesDemoSeeder
 *
...
{
    /**
     * Executa la creacio de totes les dades de demostracio dins una transaccio.
     *
     * Si qualsevol pas falla, tota la transaccio es reverteix (rollback)
     * per garantir la consistencia de les dades. L'ordre de creacio
     * respecta les dependencies de clau forana: idioma i moneda primer,
     * despres grups i tipus, opcions, i finalment productes amb variants i preus.
     *
     * @return void
     */
                ...
                foreach (['41', '42', '43'] as $s) {
                    foreach (['Negre', 'Blanc'] as $c) {
                        ...
                        Price::create([
                            ...
                            'currency_id' => $eur->id,
                            'priceable_type' => 'product_variant',  // ← Canviar de get_class($variant)
